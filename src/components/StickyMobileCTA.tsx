...
import AiHelpBotDialog, { AiHelpBotButton } from "./AiHelpBot";
    ...
    <>
      ...
      >
        <div className="flex items-stretch gap-2 p-2">
          ...
          <AiHelpBotButton onClick={() => setBotOpen(true)} />
          <button
            onClick={() => open("sticky")}
            className="flex-[1.6] flex items-center justify-center gap-1.5 rounded-lg bg-accent text-accent-foreground font-semibold text-sm py-3 shadow-sm active:scale-[0.98] transition"
          >
            <CalendarPlus className="h-4 w-4" /> Book
          </button>
        </div>
      </div>
      <AiHelpBotDialog open={botOpen} onClose={() => setBotOpen(false)} />
    </>
  );
};
